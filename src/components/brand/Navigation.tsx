...
export const Navigation = () => {
  ...
  return (
    <nav className="w-full lg:w-72 bg-rho-obsidian text-slate-100 lg:sticky lg:top-0 lg:h-screen lg:overflow-y-auto flex-shrink-0 border-r-4 border-primary z-50">
      {/* Header */}
      <div className="p-6 border-b border-slate-800 flex justify-between items-center">
        <div className="flex items-center gap-3">
          ...
          <span className="font-logo text-lg tracking-tight pt-0.5">Rhosonics</span>
        </div>
        <button
      ...
      <div className={`p-6 space-y-8 overflow-y-auto ${isOpen ? 'block' : 'hidden lg:block'}`}>
            ...
            {section.items.map((item) => (
              <button
                ...
                className={`nav-link mb-2 text-left w-full ${
                  item.highlight ? 'text-rho-green-accent' : ''
                }`}
              >
                {item.label}
              </button>
        ...
        <div className="pt-6 border-t border-slate-800">
          <div className="flex items-center gap-2">
            ...
            <span className="label-tech text-slate-500">SYSTEM V.FINAL</span>
          </div>
        </div>
      </div>
    </nav>
