...
const ExampleCard = ({ type, title, children }: ExampleCardProps) => (
  <div className={`rounded-lg border overflow-hidden ${type === 'do' ? 'border-eco-border' : 'border-red-200'}`}>
    <div className={`px-4 py-2 flex items-center gap-2 ${type === 'do' ? 'bg-eco-surface' : 'bg-red-50'}`}>
      {type === 'do' ? <Check className="w-4 h-4 text-primary" /> : <X className="w-4 h-4 text-destructive" />}
      <span className={`font-ui font-medium text-sm ${type === 'do' ? 'text-primary' : 'text-destructive'}`}>{title}</span>
    </div>
    ...
    <section id="dos-donts" className="mb-16">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-10">
        ...
        <ExampleCard type="do" title="Use approved colors">
          <div className="h-20 flex items-center justify-center bg-rho-obsidian rounded">
        ...
        <ExampleCard type="dont" title="Busy backgrounds">
          <div className="h-20 flex items-center justify-center bg-gradient-to-r from-pink-500 via-purple-500 to-indigo-500 rounded">
            <div className="w-10 h-10"><RhosonicsLogo variant="white" /></div>
          </div>
        </ExampleCard>
      </div>
      ...
      {/* General Guidelines - Two columns */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="card-base p-6">
          ...
          <ul className="space-y-2 text-sm text-muted-foreground">
            ...
            <li>• Use high contrast for data displays</li>
            <li>• Include units with all measurements</li>
            <li>• Use chamfered button shape for CTAs</li>
          </ul>
        </div>
        ...
        <div className="card-base p-6">
          ...
          <ul className="space-y-2 text-sm text-muted-foreground">
            <li>• Add drop shadows or glows to the logo</li>
            <li>• Use decorative or script fonts</li>
            <li>• Apply gradient to body text</li>
            <li>• Use Lime Accent as a standalone color</li>
          </ul>
        </div>
      </div>
    </section>
