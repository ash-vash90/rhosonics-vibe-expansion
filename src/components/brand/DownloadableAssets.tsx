...
import { buildColorCSS, FONT_CSS, colorValues, colorToken, type ColorToken } from '@/lib/brandTokens';
import { generateIconOnlySVG, logoVariants } from '@/lib/logoExport';

const generateCSSVariables = buildColorCSS;
const generateFontCSS = () => FONT_CSS;
const generateTailwindConfig = () => 'export default ' + JSON.stringify({ theme: { extend: { colors: Object.fromEntries(Object.keys(colorValues).map(key => [key, colorToken(key as ColorToken).hex])) } } }, null, 2);
...
const downloadLogo = async (variant: 'gradient' | 'white') => {
  const definition = logoVariants.find(item => item.id === (variant === 'gradient' ? 'gradient-icon-only' : 'white-icon-only'));
  if (!definition) throw new Error('Logo variant unavailable');
  downloadFile(await generateIconOnlySVG(definition), `rhosonics-logo-${variant}.svg`, 'image/svg+xml');
};

export const DownloadableAssets = () => {
  const handleDownloadAll = () => {
    void downloadLogo('gradient');
    void downloadLogo('white');
    ...
    <section id="downloads" className="space-y-16">
        ...
        <div className="bg-background p-8">
          ...
          <div className="mb-6">
            ...
            <div className="grid grid-cols-2 gap-2">
              {(['gradient', 'white'] as const).map((variant) => (
                <button
                  key={variant}
                  onClick={() => void downloadLogo(variant)}
      ...
      <BrandCallout variant="info" title="Complete Brand Package">
        <span className="flex items-center justify-between">
          <span>Logo icons, colour tokens and font CSS as separate files.</span>
          <button
            onClick={handleDownloadAll}
            className="rounded bg-action text-primary-foreground px-5 py-2.5 font-data text-xs uppercase tracking-wider hover:opacity-90 transition-opacity flex items-center gap-2 ml-4"
          >
